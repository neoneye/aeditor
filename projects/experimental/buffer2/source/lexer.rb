class LexerBase
	...
	def check_result_length(length)
		return  # TODO: remove me
		sum = 0
		@result.each do |(text, state)|
			sum += text.size
		end
		if sum != length
			p @result
			raise 'lexer result integrity error: ' +
...
class Lexer < LexerBase
	...
	def format_comment(text, code_normal, code_tab, code_end)
		re = /(\t+)|(.)|((?m).+)/
		text.scan(re) do |s2, s1, s0|
			format(s2, code_tab) if s2
			format2(s1, code_normal) if s1
			format(s0, code_end) if s0
		end
		format_end(code_end)
	end
		...
		tokens.each do |bad, good|
			...
			when /\A(=begin)(.*)/ # multiline comment
				...
				if ok
					@states << State::Comment.new
					format_comment(good, 
						:mcomment, :mcomment_tab, :mcomment_end)
				end
				ok ? nil : :bad
			...
			when /\A#/
				format_comment(good, 
					:comment, :comment_tab, :comment_end)
				nil
			when /\A["'`]/ 
				quote_begin = $~.to_s
				m = /\A['"`](?:[^\\\\]|[\\\\].)*?(['"`]?)\z/.match(good)
	...
	def lex_line_endoffile(text)
		...
		format_comment(text, 
			:endoffile, :endoffile_tab, :endoffile_end)
	end
